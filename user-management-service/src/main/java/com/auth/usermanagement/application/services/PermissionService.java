...
import com.auth.usermanagement.application.services.mapper.PermissionMapper;
 ...
 *
 * Responsabilidades:
 * - Orquestar creación/actualización de PermissionDomain via
...
public class PermissionService implements PermissionUseCasePort {
    ...
    public List<PermissionResponse> findAll() {
        return permissionRepositoryPort.findAll().stream().map(PermissionMapper::toResponse)
    ...
    public PaginatedResponse<PermissionResponse> search(String name, int page, int size) {
        ...
        List<PermissionResponse> responses = content.stream().map(PermissionMapper::toResponse)
                .collect(Collectors.toList());
