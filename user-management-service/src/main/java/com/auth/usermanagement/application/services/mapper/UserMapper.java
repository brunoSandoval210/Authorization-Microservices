...
import com.auth.usermanagement.domain.model.user.vo.*;
...
public class UserMapper {
    ...
    public static void applyUpdate(UserDomain existing, UpdateUserRequest req) {
        try {
            ...
            // aplicar email si viene (la validación de unicidad debe hacerse en el service
    ...
    public static UserResponse toResponse(UserDomain domain) {
        try {
            ...
            String namesConcat = domain.getNames() == null ? ""
                    : (domain.getNames().name() + " " + domain.getNames().lastName()
                            + (domain.getNames().secondName() == null ? "" : " " + domain.getNames().secondName()));

            List<RoleResponse> roles = domain.getRoles() == null ? List.of()
    ...
    private static RoleResponse roleToResponse(RoleDomain r) {
        try {
            ...
            List<PermissionResponse> perms = r.getPermissions() == null ? List.of()
