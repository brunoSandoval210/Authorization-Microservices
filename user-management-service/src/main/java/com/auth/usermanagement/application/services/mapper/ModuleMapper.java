...
import com.auth.usermanagement.domain.model.module.vo.ModuleIcon;
import com.auth.usermanagement.domain.model.module.vo.ModuleId;
import com.auth.usermanagement.domain.model.module.vo.ModuleName;
import com.auth.usermanagement.domain.model.module.vo.ModulePath;
    ...
    public static void applyUpdate(ModuleDomain existing, UpdateModuleRequest req) {
        try {
            if (existing == null || req == null)
                return;
            if (req.name() != null)
                existing.updateName(new ModuleName(req.name()));
            if (req.path() != null)
                existing.updatePath(new ModulePath(req.path()));
            if (req.icon() != null)
                existing.updateIcon(new ModuleIcon(req.icon()));
        } catch (Exception e) {
