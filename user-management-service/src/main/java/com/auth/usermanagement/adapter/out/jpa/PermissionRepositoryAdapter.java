...
import com.auth.usermanagement.adapter.out.jpa.mapper.PermissionJPAMapper;
import com.auth.usermanagement.adapter.out.jpa.repository.ModuleRepository;
import com.auth.usermanagement.adapter.out.jpa.repository.PermissionRepository;
...
import com.auth.usermanagement.domain.port.out.PermissionRepositoryPort;
...
public class PermissionRepositoryAdapter implements PermissionRepositoryPort {
    ...
    public List<PermissionDomain> findAll() {
        try {
            return permissionRepository.findAll().stream().map(PermissionJPAMapper::toDomain)
    ...
    public Page<PermissionDomain> searchByName(String name, org.springframework.data.domain.Pageable pageable) {
        try {
            ...
            log.error("Error al buscar permisos por nombre='{}' (normalizado) : {} - causa: {}", name, e.getMessage(),
